...
export default function QuoteSection() {
      ...
      (entries) => {
        if (entries[0].isIntersecting) {
          ...
          spans.forEach((span, i) => {
            setTimeout(() => {
              span.classList.add("text-active");
            }, i * 100); // delay từng chữ
          });
        }
      },
      { threshold: 0.3 }
  ...
  const text = "“Tình bằng hữu, sự chính trực và tinh thần tự nâng tầm hun đúc nên tinh thần của một tổ chức không ngừng học tập để kiến tạo giá trị.”"
    ...
    <section>
      <div className="relative w-full h-full bg-white">
        <div className="relative w-full h-full overflow-hidden">
          ...
          {/* text  */}
          <div className="absolute bottom-[31.5px] xl:bottom-[127px] right-1/2 translate-x-1/2">
            <div ref={textRef} className="w-[286.895px] xl:w-[1291px] text-center text-[rgba(249,249,249,0.25)] text-xl xl:text-[40px] not-italic font-bold leading-[25px] xl:leading-[59.2px] tracking-[-0.2px] xl:tracking-[-0.4px]">
              {text.split("").map((char, i) => (
                <span
                  key={i}
                  className="transition-colors duration-500 text-[rgba(249,249,249,0.25)]"
                >
                  {char}
                </span>
              ))}
